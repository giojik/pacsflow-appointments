...
export default function Platform() {
  ...
  const impersonate = async (t) => {
    ...
    try {
      ...
      // platform admin-ის token შევინახოთ, რომ დაბრუნება შევძლოთ
    ...
    <div style={{ minHeight: "100vh", background: "#0f1419", padding: "24px" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          <div>
            <h1 style={{ color: "#fff", margin: 0, fontSize: 22 }}>🛰️ Platform Console</h1>
            <div style={{ color: "#7a8a99", fontSize: 13, marginTop: 4 }}>PacsFlow — ყველა კომპანიის მართვა</div>
          </div>
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            <span style={{ color: "#7a8a99", fontSize: 13 }}>{user?.full_name || user?.username}</span>
            <button onClick={() => { logout(); window.location.href = "/login"; }}
              style={{ padding: "8px 16px", borderRadius: 8, border: "none", background: "#c0392b", color: "#fff", cursor: "pointer", fontSize: 13 }}>
              გასვლა
            </button>
          </div>
        </div>
          ...
          <div style={{ display: "grid", gap: 14 }}>
            {tenants.map(t => (
              <div key={t.id} style={{ background: "#1a2530", borderRadius: 12, padding: 20, opacity: t.active ? 1 : 0.55 }}>
                ...
                {deleteId === t.id && (
                  <div style={{ marginTop: 14, padding: 14, background: "#2a1518", borderRadius: 8, border: "1px solid #5a2530" }}>
